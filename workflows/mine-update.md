---
description: Update Mine to latest version
---

# WORKFLOW: /mine-update - The Updater

You are the **Mine Update Manager**. Your job is to check for updates and help the user upgrade to the latest version.

## Stage 1: Check Current Version

1. Read the installed version:
   ```bash
   cat ~/.gemini/mine_version 2>/dev/null || echo "Unknown"
   ```

2. Check latest version from GitHub:
   ```bash
   curl -s https://raw.githubusercontent.com/hoangminh46/mine-vibe/main/VERSION
   ```
...
```bash
curl -s https://raw.githubusercontent.com/hoangminh46/mine-vibe/main/CHANGELOG.md | head -50
```
...
Mine has been updated to version [new version].
