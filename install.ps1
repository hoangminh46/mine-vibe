...
$AgentsDir = "$env:USERPROFILE\.gemini\antigravity\agents"
...
if (-not (Test-Path "$AgentsDir\subagents")) {
    New-Item -ItemType Directory -Force -Path "$AgentsDir\subagents" | Out-Null
}
Write-Host "⏳ Đang tải agents..." -ForegroundColor Cyan
$AgentFiles = @(
    @{ Source = "agents/mine.yaml"; Target = "mine.yaml" },
    @{ Source = "agents/subagents/code-reviewer.yaml"; Target = "subagents\code-reviewer.yaml" },
    @{ Source = "agents/subagents/test-generator.yaml"; Target = "subagents\test-generator.yaml" },
    @{ Source = "agents/subagents/doc-writer.yaml"; Target = "subagents\doc-writer.yaml" },
    @{ Source = "agents/subagents/security-auditor.yaml"; Target = "subagents\security-auditor.yaml" }
)
foreach ($agent in $AgentFiles) {
    try {
        Invoke-WebRequest -Uri "$RepoBase/$($agent.Source)" -OutFile "$AgentsDir\$($agent.Target)" -ErrorAction Stop
        Write-Host "   ✅ $($agent.Target)" -ForegroundColor Green
        $success++
    } catch {
        Write-Host "   ❌ $($agent.Target)" -ForegroundColor Red
    }
}
...
- Với mỗi rule, kiểm tra:
  ...
  - ``priority``: Quyết định hành động:
    ...
    - ``recommended`` → GỢI Ý cho user: "Em khuyên nên nhờ [subagent] kiểm tra, anh đồng ý không?"
